...
let
  ...
  polyglot_android_studio =
    pkgs.writeShellScriptBin "polyglot_android_studio" ''
      #!/bin/sh
  ...
  in {
    ...
    rebuildtest =
      rebuild_cmd "test"; # also needs sudo to activate systemd services
    captiveportal =
...
in {
  ...
  home-manager = {
    ...
    users.robert = rec {
      ...
      programs.git = {
        enable = true;
        aliases = { add-commit = "!git add -A && git commit"; };
        # delta.enable = true;
